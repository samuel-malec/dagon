...
Na úvod sme si naštudovali potrebné informácie ohľadom QuickJS, t.j. ako vyzerá jeho inštrukčná sada,
aký formát má QuickJS bajtkód, ako načítať QuickJS bajtkód a spustiť ho napr. z C++ aplikácie.
Po úvodnej rešerši sme naimplementovali jednoduchý C++ program, ktorý načíta QuickJS modul a spustí ho.
Tento jednoduchý program nám bude slúžiť na spúštanie nami prekladaných QuickJS modulov.
...
[js2ct]
Nad rámec oficiálneho zadania projektu, sme sa rozhodli implementovať prekladač z podmnožiny jazyka JavaScript do Cthuhlu, ktorý sme nazvali `js2ct`. Tento smer prekladu nám umožní mať end-to-end pipeline, vďaka ktorej budeme môcť porovnávať QuickJS bajtkód vyprodukovaný priamo prekladačom qjsc, a QuickJS bajtkódom, ktorý bol analyzovaný pomocou Cthulhu.
